...
def make_saving_paths(
        destination_path: Path,
        dataset_id: int
    ) -> preprocessing.SavingPaths:

    return preprocessing.SavingPaths.make(
        destination_path,
        dataset_id
    )


def make_graph_mol_config(
    ...
    features = helpers.make_features(
        dataset_metadata.model_name
    )

    return preprocessing.ConfigData(
...
def make_entries_path_data(
    ...
    entries_paths = [
                        entries_dir / pdb 
                        for pdb in os.listdir(entries_dir)
                    ]

    entries_data = helpers.load_target_data(
        dataset_metadata.target_path
    )
...
def calc_chunck_n_proc(
        entries_paths: list[Path], 
        n_cpu: int=1
    ) -> dict[str, int]:
    
    chunk_size = helpers.chunk_size_calc(
        entries_paths,
        n_cpu
    )
...
def make_graph_manager(
    ...
    path_builder = helpers.get_path_builder(
        dataset_metadata,
        preproc_data
    )

    graph_builder = GeneralGraphBuilder(
        preproc_data.configs.graph_config, 
...
def make_graph_dataset_meta(
        preprocess_data: preprocessing.PreprocessingData,
        dataset_metadata: DatasetMetadata
    ) -> GraphDatasetMeta:

    graph_config_metadata = make_graph_metadata(
        preprocess_data.configs.features, 
        preprocess_data.configs.graph_config
    )

    return GraphDatasetMeta(
        name=dataset_metadata.name,
        model=dataset_metadata.model_name,
...
def start_convert(
    ...
    torch_convert.dataset_converter(
        tmp_data,
        graph_dataset_dest,
...
def preprocess_graph_dataset(
    ...
    preprocess_data = make_preproc_data(
        dataset_metadata,
        destination_path,
        n_cpu=n_cpu
    )
    ...
    graph_manager = make_graph_manager(
        dataset_metadata,
        preprocess_data
    )

    graph_dataset_metadata = make_graph_dataset_meta(
        preprocess_data,
        dataset_metadata
    )

    print(preprocess_data.configs.graph_config.interaction)
    ...
    graph_dataset_metadata.save(
        preprocess_data.saving_paths.metadata
    )
