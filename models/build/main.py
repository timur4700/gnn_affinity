...
def build_model(graph_dataset_directory: Path):
    ...
    model_directory = utils.make_model_directory(
    ...
    model_metadata.model_config_path = str(
        utils.model_config_save(
    ...
    model_metadata.trainer_config_path = str(
        utils.trainer_config_save(
            model_directory['main'],
            dataset_id
        ).resolve()
    )
