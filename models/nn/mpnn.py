...
class ModuleMPNN(Module):
    ...
    def forward(
        ...
        if self.edge_attr:
            x = self.gnn_conv_module(x, edge_index, edge_attr)

        else:
            x = self.gnn_conv_module(x, edge_index)


        if isinstance(self.norm, GraphNorm):
            x = self.norm(x, batch_idx)
...
class GINlayer(ModuleMPNN):
    def __init__(
        ...
        self.fc_module = Sequential(
            Linear(node_input_dim, node_input_dim), 
...
class GATLayer(ModuleMPNN):
    def __init__(
        ...
        if gat_version == 1:
            ...
            self.gnn_conv_module = GATv2Conv(
                in_channels=node_input_dim,
                out_channels=node_hidden_dim,
                heads=num_heads,
                concat=concat,
...
class EGNNConv(Module):
        def __init__(
            ...
            if edge_attr:
                self.edge_function = Sequential(
                    Linear(input_dim*4, hidden_dim), 
                    SiLU(), 
            ...
            self.coordinate_function = Sequential(
                Linear(hidden_dim, hidden_dim), 
                SiLU(), 
                Linear(hidden_dim, 1)
            )
            
            self.node_function = Sequential(
        ...
        def forward(
            ...
            if self.edge_attr:
                ...
                # [E, n*3 + 1]
                m_ij = self.edge_function(
                    torch.cat(
                        [h_i, h_j, dist, edge_attr], 
                        dim=-1
                    )
                ) 
            ...
            #[E, 3]
            summed_m_ij = torch.zeros_like(coords).scatter_add_(
                0, 
                i_expand_coord, 
                coord_update
            ) 
